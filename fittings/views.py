        ...
        if attribute.attribute_id == 1367:
            ...
            if t3c:
                for item in FittingItem.objects.filter(fit=fit).exclude(flag="Cargo"):
                    attbs = item.type_fk.dogma_attributes.filter(
                        attribute_id__in=subAttbs
                    )
                    for attb in attbs:
...
def dashboard(request):
    ...
    for doc in docs:
        doc_dict[doc.pk] = (
            doc.fittings.all().values("ship_type", "ship_type_type_id").distinct()
        )
...
def view_fit(request, fit_id):
    ...
    items = FittingItem.objects.filter(fit=fit).annotate(
...
def add_doctrine(request):
    ...
    ships = (
        Fitting.objects.order_by("ship_type")
        .values("ship_type", "ship_type__type_name")
...
def save_fit(request, token, fit_id):
    ...
    for item in fit.items.all():
        f_item = {"flag": item.flag, "quantity": item.quantity, "type_id": item.type_id}
        fit_dict["items"].append(f_item)

    # Get client
    c = esi.client
    fit = c.Fittings.post_characters_character_id_fittings(
